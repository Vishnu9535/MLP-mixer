...
def parse_args():
  ...
  parser.add_argument(
      '--target', type=str, default='Close_Price', help='target feature in S or MS task'
  )
  parser.add_argument(
      '--checkpoint_dir',
      type=str,
...
#   import pandas as pd
#   df = pd.read_csv(r'C:\Users\vishn\OneDrive\Documents\tsmixer\sample.csv')
...
#   columns_to_normalize = ['Open_Price', 'High_Price', 'Low_Price', 'Volume']  
...
#   columns_to_normalize = ['date','Open_Price', 'High_Price', 'Low_Price', 'Volume']  
#   input_features = df[columns_to_normalize]
#   # save result to csv
  import pandas as pd
  train_data = data_loader.get_train()
...
#   print(predictions)
  ...
  transformed_predictions = data_loader.inverse_transform(predictions_2d,0)
  print(transformed_predictions)
  pd.DataFrame(transformed_predictions).to_csv('predictionsf.csv', index=False)
...
#   MODEL_SAVE_DIR = r'C:\Users\vishn\OneDrive\Documents\tsmixer\tsmixer'

#   model.save(os.path.join(MODEL_SAVE_DIR, f'{exp_id}_trained_model'))
#   model.save_weights(os.path.join(MODEL_SAVE_DIR, f'{exp_id}_trained_weights'))
  if 'TSMixer' in args.model:
